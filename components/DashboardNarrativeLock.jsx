// File: components/DashboardNarrativeLock.jsx

import { useUserTier } from "./UserTierContext";
import { Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";

export function DashboardNarrativeLock({ onUnlock }) {
  const tier = useUserTier();
  if (tier === "premium") return null;

  return (
    <div className="relative rounded-xl border border-yellow-300 bg-yellow-50 px-6 py-4 shadow-inner">
      <div className="flex items-center gap-3">
        <Sparkles className="w-5 h-5 text-yellow-600 shrink-0" />
        <div className="flex-1">
          <p className="text-sm text-yellow-800">
            Ready to see your reflections evolve into strategy? Unlock the narrative synthesis.
          </p>
        </div>
        <Button
          size="sm"
